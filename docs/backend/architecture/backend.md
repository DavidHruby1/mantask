...
- Task movement relies on existing membership rather than role-based policy, does not lock whole columns, and retains count-based (not serialized) `IN_PROGRESS` capacity enforcement.
