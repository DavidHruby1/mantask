...
This module owns bootstrap, login/logout, current-session lookup, and the core user/team persistence used by those flows. Auth state is stored as a hashed session token; selection of the currently displayed team is not part of authentication state.
...
- `app/services/auth.py`: password verification and session token handling.
...
When allowed, `bootstrap_application()` creates the singleton `AppConfig`, creates the first `User` with an argon2 password hash, creates a private team and a shared team, and adds `TeamMember` rows for both with `UserRole.OWNER`.

The endpoint then creates a session, commits, sets the session cookie, and returns `BootstrapResult` with `bootstrapped=True`. The response body does not include the raw session token or a selected team.
...
On success, `LoginService.create_session()` generates a URL-safe token with `secrets.token_urlsafe(32)`, stores only its SHA-256 hex digest, and sets an expiry based on `settings.SESSION_EXPIRE_DAYS`. The endpoint commits and sends the raw token back in both the response body and an `HttpOnly`, `SameSite=lax` cookie.
...
`GET /auth/me` depends on the current session cookie. The dependency path validates the cookie, loads the session by token hash, and rejects missing or invalid sessions. The endpoint returns `LoginResult(authenticated=True)` and does not read or write team selection.

### Persistence model

`User` rows live in `app_users`. The table enforces nonblank username, normalized username, email, and password hash values, with active-user unique indexes on normalized username and email. It also tracks `is_active`, timestamps, and `profile_picture_path`, and it exposes `team_members` and `sessions` relationships.
...
2. A login request verifies credentials, creates a hashed session record, and sets the session cookie.
3. A session-backed request reads the cookie and validates the stored session.
...
- `app/repositories/teams.py`: creates teams and memberships and supports team access checks.
...
- `app/services/auth.py`: password verification and session creation/revocation.
